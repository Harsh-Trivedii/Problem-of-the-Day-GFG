Problem of the day: Distribute candies in a binary tree

Question link: https://www.geeksforgeeks.org/problems/distribute-candies-in-a-binary-tree/1

Question: Given a binary tree with N nodes, in which each node value represents number of candies present at that node. In one move, one may choose two adjacent nodes and move only one candy from one node to another (the move may be from parent to child, or from child to parent.) 
