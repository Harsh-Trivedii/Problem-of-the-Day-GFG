Problem of the day: Check whether BST contains Dead End

Question link: https://www.geeksforgeeks.org/problems/check-whether-bst-contains-dead-end/1

Question: Given a Binary Search Tree that contains unique positive integer values greater than 0. The task is to complete the function isDeadEnd which returns true if the BST contains a dead end else returns false. Here Dead End means a leaf node, at which no other node can be inserted.
